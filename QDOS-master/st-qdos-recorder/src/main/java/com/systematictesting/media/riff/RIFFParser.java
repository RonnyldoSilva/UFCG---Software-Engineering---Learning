...
import com.systematictesting.media.io.ImageInputStreamAdapter;
...
public class RIFFParser extends Object {
	...
	private void parseFORM(HashMap props) throws ParseException, AbortException, IOException {
		...
		try {
			...
			chunk.setParserMessage(
					"Unexpected EOF after " + NumberFormat.getInstance().format(getScan(in) - offset) + " bytes");
		} finally {
	...
	public static boolean isID(int id) {
		...
		return id == NULL_NUL_ID || c0 >= 0x20 && c0 <= 0x7e && c1 >= 0x20 && c1 <= 0x7e && c2 >= 0x20 && c2 <= 0x7e
				&& c3 >= 0x20 && c3 <= 0x7e;
	}
