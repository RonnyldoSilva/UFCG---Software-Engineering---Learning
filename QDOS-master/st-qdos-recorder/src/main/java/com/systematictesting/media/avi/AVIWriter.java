...
import static com.systematictesting.media.AudioFormatKeys.ENCODING_ALAW;
import static com.systematictesting.media.AudioFormatKeys.ENCODING_MP3;
...
import static com.systematictesting.media.AudioFormatKeys.ENCODING_ULAW;
...
import static com.systematictesting.media.VideoFormatKeys.ENCODING_AVI_MJPG;
import static com.systematictesting.media.VideoFormatKeys.ENCODING_AVI_PNG;
...
import com.systematictesting.media.riff.RIFFParser;
...
public class AVIWriter extends AVIOutputStream implements MovieWriter {
	...
	public final static Format VIDEO_RAW = new Format(MediaTypeKey, MediaType.VIDEO, MimeTypeKey, MIME_AVI, EncodingKey,
			ENCODING_AVI_DIB, CompressorNameKey, COMPRESSOR_NAME_QUICKTIME_RAW);
	public final static Format VIDEO_JPEG = new Format(MediaTypeKey, MediaType.VIDEO, MimeTypeKey, MIME_AVI,
			EncodingKey, ENCODING_AVI_MJPG, CompressorNameKey, COMPRESSOR_NAME_QUICKTIME_RAW);
	public final static Format VIDEO_PNG = new Format(MediaTypeKey, MediaType.VIDEO, MimeTypeKey, MIME_AVI, EncodingKey,
			ENCODING_AVI_PNG, CompressorNameKey, COMPRESSOR_NAME_QUICKTIME_RAW);
	public final static Format VIDEO_RLE = new Format(MediaTypeKey, MediaType.VIDEO, MimeTypeKey, MIME_AVI, EncodingKey,
			ENCODING_AVI_RLE, CompressorNameKey, COMPRESSOR_NAME_QUICKTIME_RAW);
	public final static Format VIDEO_SCREEN_CAPTURE = new Format(MediaTypeKey, MediaType.VIDEO, MimeTypeKey, MIME_AVI,
	...
	private int addVideoTrack(Format vf) throws IOException {
		...
		int tr = addVideoTrack(vf.get(EncodingKey), vf.get(FrameRateKey).getDenominator(),
				vf.get(FrameRateKey).getNumerator(), vf.get(WidthKey), vf.get(HeightKey), vf.get(DepthKey),
	...
	private int addAudioTrack(Format format) throws IOException {
		...
		return addAudioTrack(waveFormatTag, timeScale, sampleRate, numberOfChannels, sampleSizeInBits, isCompressed,
	...
	public void write(int track, BufferedImage image, long duration) throws IOException {
		...
		if (fmt.get(WidthKey) != image.getWidth() || fmt.get(HeightKey) != image.getHeight()) {
			throw new IllegalArgumentException(
					"Dimensions of image[" + vt.samples.size() + "] (width=" + image.getWidth() + ", height="
							+ image.getHeight() + ") differs from video format of track: " + fmt);
		}

		{
			...
			writeSample(track, (byte[]) vt.outputBuffer.data, vt.outputBuffer.offset, vt.outputBuffer.length,
	...
	private void createCodec(int track) {
		...
		if (tr.codec != null) {
			if (fmt.get(MediaTypeKey) == MediaType.VIDEO) {
				tr.codec.setInputFormat(
						fmt.prepend(EncodingKey, ENCODING_BUFFERED_IMAGE, DataClassKey, BufferedImage.class));
				if (null == tr.codec.setOutputFormat(fmt.prepend(FixedFrameRateKey, true, QualityKey,
						getCompressionQuality(track), MimeTypeKey, MIME_AVI, DataClassKey, byte[].class))) {
