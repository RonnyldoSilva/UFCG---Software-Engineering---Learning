...
public class InputUniqueEmailAddress implements Process {
	...
	public String execute(Map<String, String> elementKeyValuePairs, TestStepData testStep) {
		...
		try {
			...
			if (element!=null){
				String keysToSend = testStep.getElementValue();
				keysToSend = keysToSend.replace("${number}", System.currentTimeMillis()+"");
